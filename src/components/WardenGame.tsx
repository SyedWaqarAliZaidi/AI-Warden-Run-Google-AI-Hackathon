...
export function WardenGame() {
  ...
  const [menuTab, setMenuTab] = useState<"root" | "difficulty" | "controls" | "records" | "tutorial">("root");
  ...
  const [highScores, setHighScores] = useState({ maxLevel: 1, bestTime: 999, bestAccuracy: 0, totalCredits: 0 });
  ...
  useEffect(() => {
    ...
    if (scores) {
      try { setHighScores(JSON.parse(scores)); } catch { /* ignore */ }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (g && ctx) {
        ...
        if (phase === "playing" && !isPaused) { // Only update if not paused!
          g.update(dt);
  ...
  useEffect(() => {
    ...
    const onDown = (e: KeyboardEvent) => {
      ...
      if (["w","a","s","d","arrowup","arrowdown","arrowleft","arrowright"," ","q","e","r","f","j","z","x","c","tab","enter"].includes(k)) e.preventDefault();
      ...
      else if (k === "f" || k === "j" || k === "z" || k === "x" || k === "enter") g.pressAction("shoot");
      else if (k === "c" || k === "tab") {
  ...
  const handleDeath = (reason?: string) => {
    ...
    if (engineRef.current) {
      const g = engineRef.current;
      updateRecords(g.level, g.elapsed, g.m.shotsFired > 0 ? g.m.shotsHit / g.m.shotsFired : 0, g.m.damageDealt);
    }
  ...
  const onJoyMove = (e: React.PointerEvent) => {
    ...
    engineRef.current.virtualMove = { x: Math.cos(a) * m * sensitivity, y: Math.sin(a) * m * sensitivity };
  };
  const onJoyEnd = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-[#05060e] text-cyan-100 font-mono">
      ...
      <div className="flex flex-col items-center justify-center p-1.5 sm:p-4 w-full h-full max-w-[1000px] mx-auto flex-1">
        <div className="relative w-full max-w-full flex flex-col items-center justify-center">
          <div className="relative w-full max-w-full flex flex-col items-center justify-center" style={{ maxWidth: VIEW_W }}>
            <div 
              className="relative w-full aspect-[3/2] bg-slate-950/80 border border-cyan-500/40 rounded-md overflow-hidden" 
              style={{ 
                maxWidth: `min(${VIEW_W}px, 100%, 123vh)`,
            ...
            {phase === "playing" && !isPaused && (
              <div className="absolute top-3 left-8 pointer-events-auto z-30">
                ...
                >
                  <span className="text-[8px]">{isPaused ? "▶" : "‖"}</span> {isPaused ? "RESUME" : "PAUSE"}
                </button>
              </div>
            )}
            ...
            {phase === "playing" && (
              <div className="absolute top-3 left-1/2 -translate-x-1/2 bg-slate-950/85 border border-cyan-500/30 rounded-md px-3.5 py-1 flex items-center gap-2.5 backdrop-blur-md shadow-[0_0_15px_rgba(0,240,255,0.15)] pointer-events-none z-30 select-none">
                <span className="text-[9px] tracking-[0.2em] text-cyan-400/70 font-bold uppercase">TIME LIMIT:</span>
                <span className={`text-sm font-bold tabular-nums tracking-wider ${timeUrgent ? "text-red-500 animate-pulse" : "text-cyan-200"}`} style={{ textShadow: timeUrgent ? "0 0 8px #ff3b64" : "0 0 8px #00f0ff" }}>
                  {timeStr}
                </span>
              </div>
            )}
            ...
            {phase === "playing" && currentLevel !== 4 && (
              <div className="absolute top-[160px] right-3 pointer-events-none z-30 flex flex-col gap-1.5 items-end bg-slate-950/85 border border-cyan-500/30 rounded-md p-2 backdrop-blur-md shadow-[0_0_15px_rgba(0,240,255,0.15)] select-none min-w-[140px]">
                <span className="text-[9px] tracking-[0.2em] text-cyan-400/70 font-bold uppercase border-b border-cyan-500/30 pb-1 mb-0.5 w-full text-right">OBJECTIVES</span>
                <div className="flex justify-between items-center text-[10px] font-mono w-full">
                  <span className="text-cyan-200/80">HOSTILES:</span>
                  <span className={objHostiles.killed >= objHostiles.total && objHostiles.total > 0 ? "text-emerald-400 font-bold" : "text-cyan-400"}>
                    {objHostiles.killed} / {objHostiles.total}
                  </span>
                </div>
                <div className="flex justify-between items-center text-[10px] font-mono">
                  <span className="text-cyan-200/80">CACHES:</span>
                  <span className={objChests.opened >= objChests.total && objChests.total > 0 ? "text-emerald-400 font-bold" : "text-cyan-400"}>
                    {objChests.opened} / {objChests.total}
                  </span>
                </div>
                <div className="flex justify-between items-center text-[10px] font-mono">
                  <span className="text-cyan-200/80">GATE:</span>
                  <span className={gateOpen ? "text-emerald-400 animate-pulse font-bold" : "text-red-400"}>
            ...
            {phase === "playing" && (
              <div className="absolute bottom-3 left-3 bg-slate-950/90 border border-cyan-500/40 rounded-md p-2.5 w-60 backdrop-blur-md shadow-[0_0_20px_rgba(0,240,255,0.2)] flex flex-col gap-1.5 pointer-events-none select-none z-30">
                <div className="flex justify-between items-center text-[9px] tracking-wider text-cyan-400/80 font-bold">
                  ...
                  <span className={hpPct < 30 ? "text-red-400 animate-pulse font-bold" : "text-cyan-300"}>
                ...
                <div className="relative h-3.5 bg-cyan-950/30 border border-cyan-500/20 rounded overflow-hidden">
                  <div
                    ...
                    style={{
                      ...
                      background: hpPct < 30 ? "linear-gradient(90deg, #ff3b64, #ff0055)" : "linear-gradient(90deg, #00f0ff, #0088ff)",
                      boxShadow: hpPct < 30 ? "0 0 10px rgba(255,59,100,0.6)" : "0 0 10px rgba(0,240,255,0.6)"
                    }}
                  />
              ...
              <div className="absolute bottom-3 right-3 bg-slate-950/90 border border-cyan-500/40 rounded-md p-2.5 w-64 backdrop-blur-md shadow-[0_0_20px_rgba(0,240,255,0.2)] flex flex-col gap-1.5 pointer-events-none select-none z-30 font-mono">
                <div className="flex justify-between items-center border-b border-cyan-500/20 pb-1">
                  <div className="flex items-center gap-1">
                    <span className="text-[9px] text-cyan-400/70 font-bold uppercase">CREDITS:</span>
                    <span className="text-amber-400 font-bold text-[11px]" style={{ textShadow: "0 0 8px rgba(255, 170, 68, 0.4)" }}>
                      ¢ {currency}
                    </span>
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="text-[9px] text-cyan-400/70 font-bold uppercase">WEAPON:</span>
                    <span className={`text-[10px] font-bold ${reloading > 0 ? "text-amber-400 animate-pulse" : "text-cyan-200"}`}>
                      {!bulletsEnabled ? "SWORD (Melee)" : reloading > 0 ? "RELOAD" : `PLASMA (${ammo}/${magazine})`}
                    </span>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-1.5 text-[9px]">
                  <div className="flex items-center justify-between bg-cyan-950/30 border border-cyan-500/10 rounded px-2 py-0.5">
                    <span className={freezeActive ? "text-sky-200 animate-pulse" : "text-sky-300 font-bold"}>❄ CHRONO</span>
                    <div className="flex items-center gap-1">
            ...
            {isPaused && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/85 backdrop-blur-sm z-50 animate-fade-in">
                ...
                <div className="flex flex-col gap-3 w-64">
                  <button
                    onClick={() => { setIsPaused(false); audio.play("pause"); if (currentLevel === 4) audio.startBossMusic(); }}
                  ...
                  <button
                    onClick={() => { setIsPaused(false); retry(); }}
                    className="sao-node w-full py-3 text-cyan-200 text-xs font-bold tracking-[0.25em] hover:text-white cursor-pointer select-none"
                    onMouseEnter={() => audio.play("menu")}
                  >
                    RESTART REGION
                ...
                {menuTab === "root" && (
                  <div className="flex flex-col items-center justify-center w-full max-w-md py-4 animate-[float-sao_0.35s_ease-out]">
                    ...
                    <div className="relative w-20 h-20 mb-4 flex items-center justify-center">
                      ...
                      <span className="text-cyan-300 text-lg font-bold select-none animate-[cyber-glitch_4s_infinite]" style={{ textShadow: "0 0 10px #00f0ff" }}>Ω</span>
                    </div>

                    <h2 className="text-2xl font-bold tracking-[0.25em] text-cyan-300 mb-0.5" style={{ textShadow: "0 0 16px #00f0ff" }}>
                      NEXUS SHIFT
                    </h2>
                    ...
                    <div className="flex flex-col gap-3.5 w-full px-4">
                      <button
                        onClick={() => { setMenuTab("difficulty"); audio.play("pause"); }}
                      ...
                      <button
                        onClick={() => { setMenuTab("controls"); audio.play("pause"); }}
                        className="sao-node py-3.5 text-cyan-200 text-xs font-bold tracking-[0.3em] cursor-pointer select-none"
                        onMouseEnter={() => audio.play("menu")}
                      >
                        CONTROL INDEX
                      ...
                      <button
                        onClick={() => { setMenuTab("upgrades"); audio.play("pause"); }}
                        className="sao-node py-3.5 text-cyan-200 text-xs font-bold tracking-[0.3em] cursor-pointer select-none"
                        onMouseEnter={() => audio.play("menu")}
                      >
                        UPGRADES
                      ...
                      <button
                        onClick={() => { setMenuTab("records"); audio.play("pause"); }}
                        className="sao-node py-3.5 text-cyan-200 text-xs font-bold tracking-[0.3em] cursor-pointer select-none"
                        onMouseEnter={() => audio.play("menu")}
                      >
                        SYSTEM RECORDS
                      ...
                      <button
                        onClick={() => {
                          audio.play("death");
                          const confirmExit = window.confirm("DE-AUTHORIZE PROTOCOL: Close escape channel?");
                  ...
                  <div className="flex flex-col items-center justify-center w-full max-w-md py-4 animate-[float-sao_0.3s_ease-out]">
                    ...
                    <div className="w-full mb-6">
                      <div className="grid grid-cols-2 gap-2 mb-3">
                        {(Object.keys(DIFFICULTY_PRESETS) as Difficulty[]).map((d) => {
                          ...
                          return (
                            <button
                              key={d}
                              onClick={() => { setDifficulty(d); audio.play("pause"); }}
                      ...
                      <div className="text-[10px] text-cyan-400/80 mt-2 bg-cyan-950/20 border border-cyan-500/10 p-2.5 rounded text-left leading-relaxed">
                        ...
                        Hostile scaling factor is active. Enemy quantities are scaled to <span className="text-fuchsia-400 font-bold">×{diffPreset.countMul.toFixed(1)}</span>, baseline hit points to <span className="text-fuchsia-400 font-bold">×{diffPreset.hpMul.toFixed(1)}</span>, and deal damage at <span className="text-fuchsia-400 font-bold">×{diffPreset.dmgMul.toFixed(1)}</span> standard power.
                      </div>
                    </div>
                    <div className="flex gap-3 w-full px-4">
                      <button
                        onClick={() => { setMenuTab("tutorial"); setTutorialSlide(0); audio.play("pause"); }}
                      ...
                      <button
                        onClick={() => { setMenuTab("root"); audio.play("pause"); }}
                ...
                {menuTab === "controls" && (
                  <div className="flex flex-col items-center justify-center w-full max-w-lg py-4 animate-[float-sao_0.3s_ease-out]">
                    ...
                    <div className="w-full text-left text-xs bg-cyan-950/20 border border-cyan-500/15 p-4 rounded mb-5 space-y-4 max-h-[220px] overflow-y-auto pr-1">
                      <div>
                        <span className="text-cyan-300 font-bold block mb-1">KEYBOARD PROTOCOLS:</span>
                        <ul className="list-disc pl-4 space-y-0.5 text-cyan-200/80">
                           <li>WASD / Arrow Keys — Movement steer vectors</li>
                           <li>L-Click / F, J, Z, X or Enter — Strike / fire weapons in cursor direction</li>
                      ...
                      <div className="border-t border-cyan-500/10 pt-3">
                        <span className="text-cyan-300 font-bold block mb-2">MOBILE INTEGRATION &amp; SENSITIVITY:</span>
                        <div className="space-y-3">
                          <div className="flex flex-col gap-1">
                            <div className="flex justify-between text-[10px]">
                              <span className="text-cyan-400/80">JOYSTICK STEER MULTIPLIER:</span>
                              <span className="text-cyan-200 font-bold font-mono">{sensitivity.toFixed(2)}x</span>
                            </div>
                            <input
                              type="range"
                              min="0.5"
                          ...
                          <div className="flex flex-col gap-1">
                            <div className="flex justify-between text-[10px]">
                              ...
                              <span className="text-cyan-200 font-bold font-mono">{btnScale.toFixed(2)}x</span>
                            </div>
                            <input
                              type="range"
                              min="0.5"
                          ...
                          <div className="flex flex-col gap-1">
                            <div className="flex justify-between text-[10px]">
                              ...
                              <span className="text-cyan-200 font-bold font-mono">{(controlsAlpha * 100).toFixed(0)}%</span>
                            </div>
                            <input
                              type="range"
                              min="0.2"
                          ...
                          <div className="flex justify-between items-center bg-cyan-950/30 p-2 rounded">
                            <span className="text-[10px] text-cyan-400/80 font-bold">INVERT CONTROLS (L/R)</span>
                            <button
                              onClick={() => { setInvertControls(p => !p); audio.play("menu"); }}
                            ...
                            >
                              <div className={`w-3.5 h-3.5 rounded-full bg-white transition-transform ${invertControls ? "translate-x-5" : "translate-x-1"}`} />
                            </button>
                          </div>
                          <div className="flex items-center justify-between text-[10px] bg-cyan-950/40 p-2 rounded border border-cyan-500/5">
                            <span className="text-cyan-400/80 font-bold">CYBERNETIC AIM ASSIST (AUTO-AIM):</span>
                            <button
                              onClick={() => { setAutoAim(!autoAim); audio.play("pause"); }}
                  ...
                  <div className="flex flex-col items-center justify-center w-full max-w-md py-4 animate-[float-sao_0.3s_ease-out]">
                    ...
                    <div className="w-full bg-cyan-950/20 border border-cyan-500/15 p-4 rounded mb-6 text-xs text-left font-mono space-y-2.5">
                      <div className="flex justify-between">
                        ...
                        <span className="text-cyan-200 font-bold">LEVEL {highScores.maxLevel} / 4</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-cyan-400/70">MINIMUM CLEAR INTERVAL:</span>
                        <span className="text-cyan-200 font-bold">{highScores.bestTime === 999 ? "N/A" : `${highScores.bestTime.toFixed(1)}s`}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-cyan-400/70">MAXIMUM SHOT ACCURACY:</span>
                        <span className="text-cyan-200 font-bold">{(highScores.bestAccuracy * 100).toFixed(0)}%</span>
                      </div>
                      <div className="flex justify-between border-t border-cyan-500/10 pt-2">
                        ...
                        <span className="text-amber-300 font-bold">¢ {highScores.totalCredits}</span>
                      </div>
                    </div>
                    <button
                      onClick={() => { setMenuTab("root"); audio.play("pause"); }}
                ...
                {menuTab === "tutorial" && (
                  <div className="flex flex-col items-center justify-center w-full max-w-lg py-4 animate-[float-sao_0.3s_ease-out]">
                    ...
                    <div className="w-full text-left text-xs bg-cyan-950/30 border border-cyan-500/20 p-4 rounded mb-6 min-h-[160px] flex flex-col justify-between">
                      {tutorialSlide === 0 && (
                        <div>
                          <span className="text-cyan-300 font-bold block mb-1">01 // BASIC NAVIGATION</span>
                          <p className="text-cyan-200/80 leading-relaxed">
                            Control Aegis using standard <span className="text-cyan-400 font-bold">WASD / Arrow keys</span> (or the virtual Mobile Joystick). Steer with agility.
                            Use the <span className="text-fuchsia-400 font-bold">Space / Shift button</span> to trigger a Dash thrust. Dashing accelerates you, impacts close-range targets, and gives you crucial <span className="text-fuchsia-300 font-bold">Invincibility Frames</span> to slide through bullet patterns safely!
                          </p>
                        </div>
                      )}
                      {tutorialSlide === 1 && (
                        <div>
                          <span className="text-cyan-300 font-bold block mb-1">02 // COMBAT SYSTEM &amp; MELEE</span>
                          <p className="text-cyan-200/80 leading-relaxed">
                            Click, tap, or press the <span className="text-cyan-400 font-bold">F / J key</span> to swing your <span className="text-cyan-400 font-bold">Plasma Sword</span> or shoot towards your target. The sword swing sweeps enemies inside a wide 120° cone up to a range of 115px.
                            Your sword swing will deflect bullets, and it deals heavy damage that slices through active shield bubbles! Evolve &quot;Bullet Forge&quot; in the shop to add ranged laser bullets!
                          </p>
                        </div>
                      )}
                      {tutorialSlide === 2 && (
                        <div>
                          <span className="text-cyan-300 font-bold block mb-1">03 // CHESTS &amp; UPGRADE PROTOCOLS</span>
                          <p className="text-cyan-200/80 leading-relaxed">
                            Mainframe corridors contain glowing <span className="text-amber-400 font-bold">credits</span> and RNG <span className="text-amber-400 font-bold">Loot Chests</span>. Approach them to gain valuable Credits.
                            At the end of each stage (or upon process terminal failure), spend your credits in the <span className="text-cyan-300 font-bold">Aegis Upgrade Shop</span> to purchase extra HP capacity, Plasma Edge sword damage, bullets, magazine capacity, Chrono Bombs, and Grenades!
                          </p>
                        </div>
                      )}
                      {tutorialSlide === 3 && (
                        <div>
                          <span className="text-cyan-300 font-bold block mb-1">04 // THE ADAPTIVE WARDEN AI</span>
                          <p className="text-cyan-200/80 leading-relaxed">
                            The Warden is a complex adaptive security intelligence. Its Strategy Agent analyzes your metrics and balances group spawn compositions and counters your tactics.
                            Only Snipers can track u through walls (up to 2 wall pieces). Dash through shielded guards to damage them. Defeat the final Core form in Level 4 to escape!
                          </p>
                        </div>
                      )}
                      ...
                      <div className="flex justify-between items-center mt-4 border-t border-cyan-500/10 pt-3">
                        <button
                          ...
                          onClick={() => { setTutorialSlide(s => s - 1); audio.play("pause"); }}
            ...
            {phase === "between" && lastMetrics && (
              <Overlay>
                <h2 className="text-2xl text-cyan-300 tracking-widest mb-2 font-bold uppercase" style={{ textShadow: "0 0 10px rgba(0,240,255,0.4)" }}>LEVEL {lastMetrics.level} CLEARED</h2>
                ...
                <button onClick={nextLevel} className="neon-btn select-none cursor-pointer">ENTER LEVEL {lastMetrics.level + 1}</button>
              </Overlay>
            )}

            {phase === "won" && (
              <Overlay>
                <h2 className="text-3xl text-fuchsia-300 tracking-widest mb-3 font-bold uppercase animate-[cyber-glitch_5s_infinite]" style={{ textShadow: "0 0 16px #ff00cc" }}>
                  MAINFRAME ESCAPED
                </h2>
                <p className="text-cyan-200/80 text-sm mb-6 leading-relaxed">You outwitted The Warden and purged the main mainframe core. Security threat neutralised.</p>
                <button onClick={begin} className="neon-btn select-none cursor-pointer">RUN AGAIN</button>
              </Overlay>
            )}

            {phase === "lost" && (
              <Overlay>
                <h2 className="text-3xl text-red-400 tracking-widest mb-3 font-bold uppercase" style={{ textShadow: "0 0 16px #ff3b64" }}>
                  PROCESS TERMINATED
                </h2>
                ...
                <div className="flex gap-3 mt-1.5">
                  <button onClick={retry} className="neon-btn select-none cursor-pointer">RETRY LEVEL</button>
                  <button onClick={begin} className="neon-btn-outline select-none cursor-pointer">RESTART RUN</button>
                </div>
              </Overlay>
            )}
          </div>
          ...
          >
            <div
              ...
              style={{ boxShadow: joyLocked ? "0 0 25px rgba(255,59,100,0.4)" : "0 0 20px rgba(0,240,255,0.2)" }}
            >
              {joyLocked && <span className="text-[10px] text-red-400 font-bold tracking-widest animate-pulse pointer-events-none">LOCKED</span>}
            </div>
            <div className="grid grid-cols-2 gap-2" style={{ transform: `scale(${btnScale})`, transformOrigin: invertControls ? "left center" : "right center" }}>
              <button onPointerDown={() => engineRef.current?.pressAction("dash")}
                ...
                onMouseEnter={() => audio.play("menu")}>DASH</button>
              <button onPointerDown={() => engineRef.current?.pressAction("shoot")}
                ...
                onMouseEnter={() => audio.play("menu")}>FIRE</button>
              <button onPointerDown={() => {
                  if (engineRef.current) engineRef.current.weaponMode = engineRef.current.weaponMode === "sword" ? "gun" : "sword";
                  audio.play("menu");
                }}
                ...
                onMouseEnter={() => audio.play("menu")}>SWAP WPN</button>
              <button onPointerDown={() => engineRef.current?.pressAction("freeze")}
                ...
                onMouseEnter={() => audio.play("menu")}>❄ {freezeCharges}</button>
              <button onPointerDown={() => engineRef.current?.pressAction("grenade")}
                ...
                onMouseEnter={() => audio.play("menu")}>✸ {grenadeCharges}</button>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="w-full max-w-[780px] bg-[#04060e]/95 border border-cyan-500/35 rounded-lg p-5 mb-4 backdrop-blur-md text-left shadow-[0_0_30px_rgba(0,240,255,0.05)]">
      <div className="flex justify-between items-center mb-4 border-b border-cyan-500/20 pb-3">
        <span className="text-cyan-400 font-mono tracking-[0.2em] text-[11px] font-bold">// SYSTEM INTEGRATION PROTOCOL // AEGIS SHOP</span>
        <span className="text-amber-400 font-mono font-bold text-sm" style={{ textShadow: "0 0 10px rgba(255, 170, 68, 0.4)" }}>
          ¢ {currency}
        </span>
      </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col text-left">
                  <span className="text-[11px] font-bold text-cyan-200 tracking-wide uppercase">{up.name}</span>
              ...
              {/* Description */}
              <span className="text-[9px] text-cyan-300/70 leading-relaxed mt-1 flex-1 min-h-[32px]">{up.desc}</span>
              ...
              <button
                ...
                style={{
                  background: isMax ? "transparent" : canAfford ? `${up.color}25` : "transparent",
                  color: isMax ? "#475569" : canAfford ? up.color : "#ff3b64",
                  borderColor: isMax ? "#1e293b" : canAfford ? up.color : "#ff3b64",
                  boxShadow: !isMax && canAfford ? `0 0 10px ${up.color}25` : "none"
                }}
                onMouseEnter={() => audio.play("menu")}
              >
                {isMax ? "MAX PROTOCOL" : `BUY [¢${cost}]`}
              </button>
            </div>
          );
        })}
