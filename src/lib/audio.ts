...
class AudioBus {
    ...
    switch (name) {
      ...
      case "timeout": {
        for (let i = 0; i < 6; i++) {
          ...
          g.gain.linearRampToValueAtTime(0.25, t + i * 0.15 + 0.04);
          g.gain.exponentialRampToValueAtTime(0.001, t + i * 0.15 + 0.14);
          o.connect(g).connect(out);
          o.start(t + i * 0.15);
          o.stop(t + i * 0.15 + 0.16);
        }
        break;
      }
      case "menu": {
      ...
      case "chestOpen": {
        ...
        for (let i = 0; i < 4; i++) {
          ...
          const g = ctx.createGain();
          o.type = "triangle";
          o.frequency.value = 660 * Math.pow(1.18, i);
      ...
      case "coin": {
        ...
        o.frequency.linearRampToValueAtTime(1760, t + 0.06);
      ...
      case "freeze": {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          const g = ctx.createGain();
          o.type = "sine";
          o.frequency.setValueAtTime(1200 - i * 80, t + i * 0.04);
  ...
  startBossMusic() {
    const ctx = this.ensure();
    if (!ctx || !this.musicGain || this.bossInt !== null) return;
  ...
  stopBossMusic() {
    if (this.bossInt !== null) {
      clearInterval(this.bossInt);
      this.bossInt = null;
    }
